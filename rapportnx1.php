...
$report .= "<h1>Rapport Nexus 1 pour le {$yesterdayDate}</h1>";

// Mise à jour des requêtes pour la ligne Nexus 1
$notesCreatedTodayQuery = "SELECT notes.*, sites.name AS site_name, lignes.name AS ligne_name, services.name AS service_name, postes.name AS poste_name
                            ...
                            WHERE notes.created_at >= DATE_SUB(NOW(), INTERVAL 25 HOUR) AND lignes.name = 'Nexus 1'
...
$notesArchivedTodayQuery = "SELECT notes.*, sites.name AS site_name, lignes.name AS ligne_name, services.name AS service_name
                                ...
                                WHERE notes.archived = 1 
                                    AND notes.created_at >= DATE_SUB(NOW(), INTERVAL 25 HOUR) AND lignes.name = 'Nexus 1'
                                ORDER BY notes.id ASC";
...
$notesCreatedNotArchivedQuery = "SELECT notes.*, sites.name as site_name, lignes.name as ligne_name, services.name as service_name
                                 ...
                                 LEFT JOIN lignes ON notes.ligne_id = lignes.id
                                 LEFT JOIN services ON notes.service_id = services.id
                                 WHERE notes.archived = 0 AND DATE(notes.created_at) < CURDATE() AND lignes.name = 'Nexus 1' AND notes.poste IN ('Bobineur', 'Emballeur')
                                 ORDER BY notes.id DESC";

$memosCreatedTodayQuery = "SELECT memos.*, lignes.name as ligne_name 
                            FROM memos 
                            JOIN lignes ON memos.ligne_id = lignes.id
                            WHERE memos.created_at >= DATE_SUB(NOW(), INTERVAL 25 HOUR) AND lignes.name = 'Nexus 1'
...
$pdfFileName = 'rapportnexus1_' . date('Ymd', strtotime('-1 day')) . '.pdf';
...
$mail->SMTPSecure = 'tls';
$mail->Port = 25;
$mail->setFrom('rapportprod@mphygiene.com', 'Rapport Quotidien Nexus 1');
$mail->addAddress('antony.vallin@mphygiene.com', 'Antony Vallin');
$mail->isHTML(true);
$mail->Subject = "Rapport quotidien - Nexus 1 - Date: {$yesterdayDate}";
$mail->CharSet = 'UTF-8';
$mail->AddAttachment($pdfFilePath);
$mail->Body = "Bonjour<br>Ci-Joint le rapport quotidien pour la Nexus 1 de la journée du {$yesterdayDate} ";
